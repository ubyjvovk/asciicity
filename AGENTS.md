...
- `public/data/<city>/` (london/kyiv/sf/nyc — `index.json` + `tiles/*.json`,
  wave 11) — GENERATED by `npm run fetch-data[:city]` (`--tiles`) or
  `scripts/tile-city.mjs`; never hand-edit, never `cat` them (megabytes) —
  inspect with `node -e` or `jq`.
