...
## Next actions
1. Board empty. PM look-tune next (C3, `pm/tune` worktree + `node_modules/.pm-shot.mjs`): fog density for terrain cities (0.0018 → try 0.0009) so hilltop vistas (Lavra → left bank, Motherland → river) read as depth; verify the void edge beyond the grid stays hidden and London is untouched.
