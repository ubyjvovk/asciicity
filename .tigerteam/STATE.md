...
## Next actions
1. ~~Review T-0049~~ done. Next: review T-0050 (GPU check: `/?city=kyiv&at=maidan&fly=1`, hold Space then W; fog thinning; MODE/ALT rows), push. Then T-0050 (GPU: R cycle, toast, 12 stubs), then the styles — review each style's `e2e/__shots__/style-<id>.png` artifact **and** take a GPU frame from Maidan (`?city=kyiv&render=<id>`); fps ≥ 30 is a hard criterion (check `edges` — depth texture — and `braille` — 2×4 sub-samples — first).
